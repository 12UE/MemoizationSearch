...
namespace memoizationsearch {
    ...
    template<typename T>
    static inline bool ReadElementFromFile(std::ifstream& file, T& value) {//对于基本类型的读取
    ...
    template<typename T>
    static inline bool WriteElementToFile(std::ofstream& file, const T& value) {//对于基本类型的写入
    ...
    template<typename T, typename U>
    static inline bool ReadPairFromFile(std::ifstream& file, std::pair<T, U>& pair) { //对于pair的读取
    ...
    static inline typename std::enable_if<I == sizeof...(Tp), bool>::type ReadTupleImpl(std::ifstream&, std::tuple<Tp...>&)noexcept {//对于tuple的读取
    ...
    template<std::size_t I = 0, typename... Tp>
    static inline typename std::enable_if < I<sizeof...(Tp), bool>::type ReadTupleImpl(std::ifstream& file, std::tuple<Tp...>& t)noexcept {//对于tuple的读取的内部实现
    ...
    template<typename... Args>
    static inline bool ReadTupleFromFile(std::ifstream& file, std::tuple<Args...>& tuple) noexcept {//对于tuple的读取
    ...
    template<std::size_t I = 0, typename... Tp>
    static inline typename std::enable_if<I == sizeof...(Tp), bool>::type WriteTupleImpl(std::ofstream&, const std::tuple<Tp...>&) noexcept { //对于tuple的写入
    ...
    template<std::size_t I = 0, typename... Tp>
    static inline typename std::enable_if < I< sizeof...(Tp), bool>::type WriteTupleImpl(std::ofstream& file, const std::tuple<Tp...>& t) noexcept {//对于tuple的写入的内部实现
    ...
    template<>
    SAFE_BUFFER inline bool ReadElementFromFile<std::string>(std::ifstream& file, std::string& value) {//对于string的读取的特化
    ...
    template<>
    SAFE_BUFFER inline bool ReadElementFromFile<std::wstring>(std::ifstream& file, std::wstring& value) {//对于wstring的读取的特化
    ...
    template<>
    SAFE_BUFFER inline bool WriteElementToFile<std::string>(std::ofstream& file, const  std::string& value) {//对于string的写入的特化
    ...
    template<>
    SAFE_BUFFER inline bool WriteElementToFile<std::wstring>(std::ofstream& file, const std::wstring& wstr) {//对于wstring的写入的特化
    ...
    namespace nonstd {
        ...
        template<typename T>
        class ObjectPool :public SingleTon<ObjectPool<T>> {//单例奇异递归模板模式
        ...
        struct CachedFunction : public CachedFunctionBase {//具有参数的缓存函数是一个继承自CachedFunctionBase的类
            ...
            inline CachedFunction(const std::initializer_list<std::pair<ArgsType, ValueType>>& list) : CachedFunctionBase(nullptr, CallType::cdeclcall, MEMOIZATIONSEARCH), m_Cache(std::make_unique<CacheType>()) {
                ...
                if (LIKELY((bool)m_Cache))m_Cache->reserve(MEMOIZATIONSEARCH);//预先分配空间
            }
            ...
            inline CachedFunction& operator+(const CachedFunction& others) noexcept {//合并缓存重载+操作符
                ...
                auto nowtime = MemoroizationGetCurrentTime();//获取当前时间
            ...
            SAFE_BUFFER inline R& operator()(const Args&... args)noexcept {
                ...
                const TimeType& nowtime = MemoroizationGetCurrentTime();//获取当前时间
            ...
            inline bool operator<<(std::ifstream& file) noexcept {//从文件读取缓存
                ...
                while (file) {
                    ...
                    if (value.second > MemoroizationGetCurrentTime())m_Cache->insert(std::make_pair(key, value));//仅当缓存的时间大于当前时间的时候插入
                }
                for (auto it = m_Cache->begin(); it != m_Cache->end(); ) {
        ...
        //萃取函数的返回值和参数
        template <typename F>
        struct function_traits : function_traits<decltype(&F::operator())> {};//当F可能是一个lambda表达式的时

        template <typename R, typename... Args>
        struct function_traits<R(*)(Args...)> {//函数指针的萃取，具有可变参数
        ...
        template <typename R>
        struct function_traits<R(*)()> {//函数指针的萃取，没有参数
        ...
        template <typename R, typename... Args>
        struct function_traits<std::function<R(Args...)>> {//std::function的萃取，具有可变参数
        ...
        template <typename R>
        struct function_traits<std::function<R()>> {//std::function的萃取，没有参数
...
#ifdef _WIN64
        template <typename F>
        struct stdcallfunction_traits : stdcallfunction_traits<decltype(&F::operator())> {};//stdcall的lambda表达式萃取

        template <typename R, typename... Args>
        struct stdcallfunction_traits<R(STDCALL*)(Args...)> :public function_traits<R(STDCALL*)(Args...)> {};//stdcall的函数指针萃取，具有可变参数

        template <typename R>
        struct stdcallfunction_traits<R(STDCALL*)()> :public function_traits<R(STDCALL*)()> {};//stdcall的函数指针萃取，没有参数

        template <typename ClassType, typename R, typename... Args>
        struct stdcallfunction_traits<R(STDCALL ClassType::*)(Args...)> :public function_traits<R(STDCALL ClassType::*)(Args...)> {};//stdcall的成员函数萃取，具有可变参数

        template <typename ClassType, typename R>
        struct stdcallfunction_traits<R(STDCALL ClassType::*)()> :public function_traits<R(STDCALL ClassType::*)()> {};//stdcall的成员函数萃取，没有参数

        template <typename ClassType, typename R, typename... Args>
        struct stdcallfunction_traits<R(STDCALL ClassType::*)(Args...) const> :public function_traits<R(STDCALL ClassType::*)(Args...) const > {};//stdcall的成员函数萃取，具有可变参数

        template <typename ClassType, typename R>
        struct stdcallfunction_traits<R(STDCALL ClassType::*)() const> :public function_traits<R(STDCALL ClassType::*)() const > {};//stdcall的成员函数萃取，没有参数

        template <typename F>
        struct fastcallfunction_traits : fastcallfunction_traits<decltype(&F::operator())> {};//fastcall的lambda表达式萃取

        template <typename R, typename... Args>
        struct fastcallfunction_traits<R(FASTCALL*)(Args...)> :public function_traits<R(FASTCALL*)(Args...)> {};//fastcall的函数指针萃取，具有可变参数

        template <typename R>
        struct fastcallfunction_traits<R(FASTCALL*)()> :public function_traits<R(FASTCALL*)()> {};//fastcall的函数指针萃取，没有参数

        template <typename ClassType, typename R, typename... Args>
        struct fastcallfunction_traits<R(FASTCALL ClassType::*)(Args...)> :public function_traits<R(FASTCALL ClassType::*)(Args...)> {};//fastcall的成员函数萃取，具有可变参数

        template <typename ClassType, typename R>
        struct fastcallfunction_traits<R(FASTCALL ClassType::*)()> :public function_traits<R(FASTCALL ClassType::*)()> {};//fastcall的成员函数萃取，没有参数

        template <typename ClassType, typename R, typename... Args>
        struct fastcallfunction_traits<R(FASTCALL ClassType::*)(Args...) const> :public function_traits<R(FASTCALL ClassType::*)(Args...) const> {};//fastcall的成员函数萃取，具有可变参数

        template <typename ClassType, typename R>
        struct fastcallfunction_traits<R(FASTCALL ClassType::*)() const> :public function_traits <R(FASTCALL ClassType::*)() const> {};//fastcall的成员函数萃取，没有参数
        ...
        inline static auto& GetCachedFunction(void* funcAddr, CallType type,std::function<R(Args...)>&& func, TimeType cacheTime = MEMOIZATIONSEARCH) noexcept {
            ...
            return  ObjectPool<CachedFunction<R, Args...>>::Construct()(cacheTime, funcAddr, type, std::forward<std::function<R(Args...)>>(func), cacheTime);//根据参数和类型构造一个缓存函数
