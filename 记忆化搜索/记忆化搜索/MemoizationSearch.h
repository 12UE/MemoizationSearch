...
using CacheValidTimeType = unsigned long long;
 auto INFINITYCACHE = static_cast<CacheValidTimeType>(0x0000'FFFF'FFFF'FFFF'FFFFULL);//定义缓存的最大时间
#else
using CacheValidTimeType = unsigned long;
 auto INFINITYCACHE = static_cast<CacheValidTimeType>(0xFFFF'FFFF);//定义缓存的最大时间在32位下
#endif // _WIN64
...
#endif
#ifdef _DEBUG
 static std::recursive_timed_mutex logmtx;
 struct AutoLog {
     std::string funcname;//存储函数名字
     SAFE_BUFFER inline AutoLog(const std::string& szName = "", const std::string& perfix = "") noexcept {
         std::unique_lock<decltype(logmtx)> lock(logmtx);
         funcname = szName + " " + perfix;//函数的名字拼接上前缀
#ifdef DEBUG_LOG
         std::cout << funcname << "---> Begin" << std::endl;//当构造的时候会自动打印函数开始 
#endif
     }
     SAFE_BUFFER inline  ~AutoLog() noexcept {//当对象析构的时候会自动调用
#ifdef DEBUG_LOG
         std::unique_lock<decltype(logmtx)> lock(logmtx);
         std::cout << funcname << "---> End" << std::endl;//当对象析构时候自动打印结束
#endif
     }
 };
#define AUTOLOG  AutoLog log(__FUNCTION__,"");//直接打印函数名字
#define AUTOLOGPERFIX(STR)  AutoLog log(__FUNCTION__,STR);//打印函数名字但是加上一些后缀 __FUNCTION__是MSVC下的宏用于获取函数名字   MSVC的扩展都是双下划线开头的
#else
#define AUTOLOG             //release 模式下什么也不打印
#define AUTOLOGPERFIX(STR)
#endif
namespace std {
    template<typename U>SAFE_BUFFER static inline  size_t hasher(const U& value) noexcept {//哈希器，用于对任意类型进行哈希
    ...
    template<typename... T>struct TupleHasher {
        ...
        SAFE_BUFFER static inline size_t hashImpl(Tuple&& t, const index_sequence<I...>&, size_t seed = 0) noexcept {//对于tuple的hash的内部实现
            ...
            (void)expander {0, ((seed ^= hasher(get<I>(std::forward<Tuple>(t))) + 0x92E2194B + (seed << 6) + (seed >> 2)), 0)...};//异或哈希
            return seed;//返回seed seed是一个hash值
        }
        static SAFE_BUFFER inline size_t Seed()noexcept { AUTOLOG return 0x92E2194B; }//返回内部的种子
    };
...
namespace memoizationsearch {
    ...
    template<typename T>std::atomic<T*> SingleTon<T>::instancePtr(nullptr);
    ...
    static SAFE_BUFFER inline auto EachItemImpl(const F& bin, const Tuple& tup,const std::index_sequence<I...>&) {
        AUTOLOGPERFIX(typeid(F).name())//自动记录日志
        return std::make_tuple(bin(std::get<I>(tup))...);//对tuple的每个元素调用f 得到一个新的tuple 其中存每个元素调用f的结果
    }
    //对tuple的每个元素调用f 得到一个新的tuple 其中存每个元素调用f的结果
    template<typename F, typename... Args,typename Index = std::index_sequence_for<Args...>>
    static SAFE_BUFFER inline auto EachItem(const F& bin, const std::tuple<Args...>& tup) {
        AUTOLOGPERFIX(typeid(F).name())//自动记录日志
        static Index index{};//生成一个序列
        return EachItemImpl(bin, tup, index);//展开tuple
    }
    ...
    template<typename... Args>static inline bool WriteTupleToFile(std::ofstream& file, const std::tuple<Args...>& tuple) noexcept { return WriteTupleImpl(file, tuple); }//对于tuple的写入
    template<>inline bool ReadElementFromFile<std::string>(std::ifstream& file, std::string& value) {//对于string的读取的特化
    ...
    template<>inline bool ReadElementFromFile<std::wstring>(std::ifstream& file, std::wstring& value) {//对于wstring的读取的特化
    ...
    template<>inline bool WriteElementToFile<std::string>(std::ofstream& file, const  std::string& value) {//对于string的写入的特化
        AUTOLOGPERFIX(typeid(std::string).name())//自动记录日志
        if (!file) return false;//文件打开失败
    ...
    template<>inline bool WriteElementToFile<std::wstring>(std::ofstream& file, const std::wstring& wstr) {//对于wstring的写入的特化
        AUTOLOGPERFIX(typeid(std::wstring).name())//自动记录日志
        if (!file) return  false;     //文件打开失败
    ...
    namespace nonstd {
        ...
        thread_local static CacheValidTimeType count = 0;
        struct ScopeLock {//一种自动加锁的RAII模式
            explicit inline ScopeLock(std::recursive_mutex& mtx) : m_mutex(mtx), m_locked(false) { AUTOLOG  lock(); }
            inline ~ScopeLock() { AUTOLOG  unlock(); }
            SAFE_BUFFER inline void lock() const noexcept { AUTOLOG if (!m_locked && m_mutex.try_lock())m_locked = true; }
            SAFE_BUFFER inline void unlock()const noexcept { AUTOLOG if (m_locked) m_mutex.unlock(), m_locked = false; }
        ...
        template<typename T>class ObjectPool :public SingleTon<ObjectPool<T>> {//单例奇异递归模板模式
            ...
            CacheValidTimeType Size = 0;//对象池的大小
            CacheValidTimeType offset = 0;//偏移 当有对象构造的时候会偏移
        ...
        static CacheValidTimeType timeoffsetpice = 0;
        SAFE_BUFFER static inline CacheValidTimeType approximategetcurrenttime() noexcept {
            ...
            if (LIKELY(count / 4 != 0)) {//每四次更新一次时间
                ...
                static auto Update = [&]() {
                    ...
                    currenttime = newtime;//更新时间
                    if (count > INFINITYCACHE - 1) count = 0;//达到最大值的时候清零
                };
                std::thread(Update).detach();//异步更新时间
            }
            return currenttime;//返回更新后的时间
#endif
        }
        template<typename T>
         SAFE_BUFFER static inline T safeadd(const T& a, const T& b) noexcept {//安全的相加不会溢出 最大只会返回INFINITYCACHE
            AUTOLOGPERFIX(typeid(T).name())//自动记录日志
            return (LIKELY(b > INFINITYCACHE - a)) ? INFINITYCACHE : a + b;
        }
        ...
        struct CachedFunctionBase{
            ...
            mutable CacheValidTimeType m_cacheTime;//缓存的时间
            ...
            inline  CachedFunctionBase(void* funcAddr, CallType type, CacheValidTimeType cacheTime = MEMOIZATIONSEARCH) : m_funcAddr(funcAddr), m_callType(type), m_cacheTime(cacheTime) { AUTOLOG }
            inline virtual ~CachedFunctionBase(){ AUTOLOG }//析构函数自动记录日志
            inline void setcachetime(CacheValidTimeType cacheTime) noexcept {//设置缓存的有效时间
                AUTOLOG//自动记录日志
                m_cacheTime = cacheTime;//设置缓存的时间
            }//设置缓存的时间
            inline CacheValidTimeType getcachetime() const { AUTOLOG  return m_cacheTime; }//获取缓存的时间
            ...
            SAFE_BUFFER inline std::string GetPlatformName() const noexcept {
                AUTOLOG//自动记录日志
                    //判断平台
#if defined(_NTDDK_)
                    return "Windows_Kernel";//Windows内核
#elif defined(_WIN64)
                    return "Windows_64-bit";//Windows 64位
#elif defined(_WIN32)
                    return "Windows_32-bit";//Windows 32位
        ...
        //比较两个元素的相等与否 T大于指针大小的求hash
        template<typename T>SAFE_BUFFER static inline bool compareitem(const T& itemA, const T& itemB)noexcept {
            AUTOLOG//自动记录日志
             if constexpr (sizeof(T) == sizeof(uintptr_t)) {//如果是指针大小的类型直接比较
                 return itemA == itemB;
             }else {
                 return std::hasher(itemA) == std::hasher(itemB);//否则求hash
             }
        }
        //申明键的类型位可变的参数的tuple
        ...
        SAFE_BUFFER static inline bool const CompareTuple(Tuple&& tupA, Tuple&& tupB) noexcept {
            ...
            if constexpr (UNLIKELY(I == std::tuple_size<std::decay_t<Tuple>>::value)) {//判断是否到达末尾 std::decay_t<>是让tuple处于退化的状态方便std::tuple_size统计个数
                return true;//到达递归基返回 tuple中的所有元素都比较了一次都完全相等 最根部返回true
            }else {//对于&& 的类型 要用std::forward<>模板转换为本身
                return (std::get<I>(std::forward<Tuple>(tupA)) == std::get<I>(std::forward<Tuple>(tupB))) ? CompareTuple<I + 1, Args...>(std::forward<Tuple>(tupA), std::forward<Tuple>(tupB)) : false;//没到末尾就继续递归比较下一个I+1是在下一层get用的
            }
        }
        ...
        template<typename R, typename... Args>struct CachedFunction : public CachedFunctionBase {//具有参数的缓存函数是一个继承自CachedFunctionBase的类
            ...
            using CacheType = std::unordered_map<KeyType<Args...>, ValueType>;//一个哈希表存储缓存类型
            ...
            mutable CacheValidTimeType m_nowtime;//当前时间 用于判断缓存是否过期
            ...
            mutable KeyType<Args...> staticargstuple{};//记录上一次参数的tuple
            ...
            inline CachedFunction(const std::initializer_list<std::pair<KeyType<Args...>, ValueType>>& list) : CachedFunctionBase(nullptr, CallType::cdeclcall, MEMOIZATIONSEARCH), m_cache(std::make_unique<CacheType>()) {
                AUTOLOG//自动记录日志
            ...
            inline void setcachetime(CacheValidTimeType cacheTime) noexcept {
                AUTOLOG//自动记录日志
                ScopeLock lock(m_mutex);
                auto offset = cacheTime - m_cacheTime;
                for (auto& item : *m_cache)item.second.second += offset;
                m_nowtime = approximategetcurrenttime();
                for (auto it = m_cache->begin(); it != m_cache->end(); it = (it->second.second < m_nowtime) ? m_cache->erase(it) : ++it) {}//删除过期的缓存
            ...
            SAFE_BUFFER inline R& asyncspdatecache(const KeyType<Args...>& argsTuple) const noexcept {//异步更新缓存
                AUTOLOG
                auto ret = Apply(m_func, argsTuple);//调用函数
                ScopeLock lock(m_mutex);//加锁
                std::thread([this]() {
                    AUTOLOGPERFIX("INSERT")//自动记录日志
                    ScopeLock lock(m_mutex);//加锁
                    for (auto it = m_cache->begin(); it != m_cache->end(); it = (it->second.second < m_nowtime) ? m_cache->erase(it) : ++it) {}//删除过期的缓存
                ...
                auto &retref= m_cacheinstance->insert_or_assign(argsTuple, ValueType{ ret, safeadd(m_nowtime, (CacheValidTimeType)m_cacheTime) }).first->second.first;//插入或者更新缓存
                m_cacheend = m_cacheinstance->end();//更新迭代器
                staticiter = m_cacheend;//更新静态迭代器
            ...
            SAFE_BUFFER inline R& operator()(const Args&... args) const noexcept {
                ...
                if (m_cacheinstance->empty() ||!CompareTuple(staticargstuple,argsTuple)|| staticiter == m_cacheend) {//前期不做时间判断
                    staticiter = m_cacheinstance->find(argsTuple);//查找缓存
                    m_cacheend = m_cacheinstance->end();//更新迭代器
            ...
            inline void cleancache(const KeyType<Args...>& parameters) const noexcept {//清空指定的缓存 指定的参数必须是tuple
                AUTOLOG//自动记录日志
                ScopeLock lock(m_mutex);
                auto it = m_cache->find(parameters);
                if (LIKELY(it != m_cache->end()))  m_cache->erase(it->first);
            }
            inline void setcacheresettime(const KeyType<Args...>& parameters, CacheValidTimeType cacheTime) const noexcept {//设置指定的参数的缓存的时间
                AUTOLOG//自动记录日志
                ScopeLock lock(m_mutex);
                auto it = m_cache->find(parameters);
            ...
            inline bool savecache(const char* szFileName)noexcept {//将缓存保存到文件
                AUTOLOG//自动记录日志
                std::ofstream file(GetUniqueName(m_func,szFileName), std::ios::binary | std::ios::trunc);
            ...
            inline bool loadcache(const char* szFileName)noexcept {//从文件加载缓存
                AUTOLOG//自动记录日志
                std::ifstream file(GetUniqueName(m_func, szFileName), std::ios::binary);
                if (!operator<<(file)) return false;
                file.close();
                return true;
            }
            ...
            inline void operator>>(const CachedFunction& others) noexcept { 
                AUTOLOG//自动记录日志
                for (const auto& pair : *m_cache)others.m_cache->insert(pair); 
            }
            inline void operator<<(const CachedFunction& others) noexcept {
                AUTOLOG//自动记录日志
                ScopeLock lock(m_mutex);
            ...
            friend inline std::ostream& operator<<(std::ostream& os, const CachedFunction& func)noexcept {
                AUTOLOG//自动记录日志
                return os << func.m_cache->size() << std::endl;
            }
            inline std::function<FuncType>* operator->()noexcept { AUTOLOG return &m_func; }
            inline bool empty()const noexcept { AUTOLOG return m_cache->empty(); }
            inline std::size_t size()const noexcept { AUTOLOG return m_cache->size(); }
            SAFE_BUFFER inline iterator operator[](const KeyType<Args...>& key) noexcept {
                AUTOLOG//自动记录日志
        ...
        template<typename R> struct CachedFunction<R> : public CachedFunctionBase {//没有参数的缓存函数是一个继承自CachedFunctionBase的类
            ...
            inline CachedFunction(void* funcAddr, CallType type, const std::function<R()>& func, CacheValidTimeType cacheTime = MEMOIZATIONSEARCH) : CachedFunctionBase(funcAddr, type, cacheTime), m_func(std::move(func)), m_cache(std::make_pair(R{}, 0)) { AUTOLOG }
...
#endif // _WIN64
        template <typename R, typename... Args> inline static auto& GetCachedFunction(void* funcAddr, CallType type, const std::function<R(Args...)>& func, CacheValidTimeType cacheTime = MEMOIZATIONSEARCH) noexcept {
        ...
        template <typename R> inline static auto GetCachedFunction(void* funcAddr, CallType type, const std::function<R()>& func, CacheValidTimeType cacheTime = MEMOIZATIONSEARCH) noexcept {
            AUTOLOGPERFIX(typeid(std::function<R()>).name())//自动记录日志
            return  CachedFunction<R>(funcAddr, type, func, cacheTime);//根据参数和类型构造一个缓存函数
        }
        template<typename F, std::size_t... Is, typename DecayF = std::decay_t<F>> inline static auto MakeCachedImpl(F&& f, CallType type, CacheValidTimeType validtime, const std::index_sequence<Is...>&) noexcept {
            AUTOLOGPERFIX(typeid(F).name())//自动记录日志
...
//缓存函数的创建的加强版
template<typename F>inline  auto MakeCacheEx(F&& f, const  memoizationsearch::nonstd::CallType& calltype= memoizationsearch::nonstd::CallType::cdeclcall, CacheValidTimeType _validtime=MEMOIZATIONSEARCH)noexcept {
    AUTOLOGPERFIX(typeid(F).name())
    auto validtime = memoizationsearch::nonstd::Clamp(_validtime, (CacheValidTimeType)1, INFINITYCACHE);//有效时间 相加不会溢出 最大只会返回INFINITYCACHE
#ifdef _WIN64
...
#endif // _WIN64
    return  memoizationsearch::nonstd::MakeCachedImpl(f, calltype, validtime, memoizationsearch::ArgsType_v<typename  memoizationsearch::nonstd::function_traits<std::decay_t<F>>::args_tuple_type>{});//根据参数和类型构造一个缓存函数
}
template<typename R, typename...Args>  inline auto& MakeCacheItem(const R& ret, CacheValidTimeType validtime, const Args&...args) noexcept { 
    AUTOLOG//自动记录日志
    return std::make_pair(std::make_tuple(args...), std::make_pair(ret, validtime)); 
}
template<typename F> inline auto MakeCache(F&& f, const CacheValidTimeType validtime = MEMOIZATIONSEARCH) noexcept {//创建缓存 第二个参数是缓存的有效时间 默认是MEMOIZATIONSEARCH
    AUTOLOGPERFIX(typeid(F).name())//自动记录日志
    return MakeCacheEx(f, memoizationsearch::nonstd::CallType::cdeclcall, validtime);//有默认参数的函数写清楚是为了把validtime传递给他
}
template<typename T, class F> inline auto CacheMemberFunction(T&& obj, F&& func,const CacheValidTimeType validtime = MEMOIZATIONSEARCH) noexcept { //缓存成员函数
    AUTOLOG//自动记录日志
